...
from HP import baseline
from HP import magicgraph
from HP import skipgram
from HP.magicgraph import WeightedDiGraph, WeightedNode
...
class DoubleWeightedDiGraph(WeightedDiGraph):
    ...
    def subgraph(self, nodes=list(), index_mapping=None):
        nodes = set(nodes)
...
def external_ec_coarsening(graph, sfdp_path, coarsening_scheme=2):
    ...
    subprocess.call('%s -g%d -v -u -Tc %s 2>x' % (sfdp_abs_path, coarsening_scheme, input_fname), shell=True,
                    cwd=temp_dir)
...
def skipgram_coarsening_disconnected(graph, recursive_graphs=None, recursive_merged_nodes=None, **kwargs):
    ...
    num_workers = kwargs.get('workers',1)
    ...
    for subgraph, reversed_mapping in zip(subgraphs, reversed_mappings):
        ...
        if not subgraph.is_connected():
            gc_single_model = baseline.skipgram_baseline(subgraph,
                                                         ...
                                                         alpha=alpha,
                                                         min_alpha=min_alpha,workers=kwargs['workers'],
                                                         sg=1,min_count=kwargs['min_count'],sample=kwargs['sample'],negative=kwargs['negative'],
                                                         hs=hs,)
            ...
            print("coarsened to %d levels" % len(recursive_graphs))
            ...
            if hs == 1:
                gc_model = skipgram_coarsening_hs(recursive_graphs, recursive_merged_nodes,
                                                  ...
                                                  sg=1,
                                                  hs=1,
                                                  sample=sample,min_count=kwargs['min_count'],
                ...
                gc_model = skipgram_coarsening_neg(recursive_graphs, recursive_merged_nodes,
                                                   ...
                                                   alpha=alpha,
                                                   min_alpha=min_alpha,
                                                   sample=sample,min_count=kwargs['min_count'],negative=kwargs['negative'],
                                                   sg=1,
                                                   hs=0,
                                                   num_workers=num_workers)
...
def skipgram_coarsening_hs(recursive_graphs, recursive_merged_nodes, **kwargs):
    ...
    for level in range(levels - 1, -1, -1):
        if scale == 1:
            ...
            edges = build_deepwalk_corpus(recursive_graphs[level], num_paths, path_length, output, num_workers=kwargs['num_workers'])

        # the coarest level
        if level == levels - 1:
            model = skipgram.Word2Vec_hs_loss(edges, sg=kwargs['sg'], size=kwargs['representation_size'],
                                              iter=kwargs['iter'][level], window=kwargs['window_size'], sample=sample,
                                              alpha=alpha_list[level], min_alpha=min_alpha_list[level], workers=kwargs['num_workers'])
        else:
            model = skipgram.Word2Vec_hs_loss(None, sg=kwargs['sg'], size=kwargs['representation_size'],
                                              iter=kwargs['iter'][level], window=kwargs['window_size'], sample=sample,
                                              alpha=alpha_list[level], min_alpha=min_alpha_list[level], workers=kwargs['num_workers'])
            ...
            cur_merged_nodes = [(node, merged_node) for node, merged_node in recursive_merged_nodes[level].items()
                                if node != merged_node]
...
def skipgram_coarsening_neg(recursive_graphs, recursive_merged_nodes, **kwargs):
    ...
    models = []
    original_graph = recursive_graphs[0]
    # print(f'{len(original_graph.nodes())} nodes in original graph')
    levels = len(recursive_graphs)
    ...
    for level in range(levels - 1, -1, -1):
        total_walk_entries += len(recursive_graphs[level].nodes()) * kwargs.get('path_length', 10) * kwargs.get('num_paths', 40)
        # DeepWalk
        if scale == -1:
            path_length = kwargs.get('path_length', 10)
            num_paths = kwargs.get('num_paths', 40)
        ...
        # the coarest level
        if level == levels - 1:
            model = Word2Vec(edges, size=kwargs['representation_size'], window=kwargs['window_size'],min_count=kwargs['min_count'],negative=kwargs['negative'],
                             sample=sample, sg=1, alpha=alpha_list[level], min_alpha=min_alpha_list[level], hs=0, iter=kwargs['iter'][level], workers=kwargs['num_workers'])
        else:
            model = Word2Vec(None, size=kwargs['representation_size'], window=kwargs['window_size'],min_count=kwargs['min_count'],negative=kwargs['negative'],
                             sample=sample, sg=1, alpha=alpha_list[level], min_alpha=min_alpha_list[level], hs=0, iter=kwargs['iter'][level], workers=kwargs['num_workers'])
...
def build_deepwalk_corpus(G, num_paths, path_length, output, alpha=0,num_workers=1):
    walks_filebase = output + '.walks'
    walk_files = serialized_walks.write_walks_to_disk(G, walks_filebase, num_paths=num_paths,
                                                      path_length=path_length, alpha=alpha,
                                                      rand=random.Random(random.randint(0, 2 ** 31)),
                                                      num_workers=num_workers)
