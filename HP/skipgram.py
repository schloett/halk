...
class Word2Vec_hs_loss(Word2Vec):
    ...
    def add_word(self, word, parent_word, emb, cur_index):
        ...
        if word != parent_word:
            ...
            self.wv.vocab[word].point = array(list(self.wv.vocab[word].point) + [self.inner_node_index_map[word]],
                                           dtype=uint32)
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        if job_tally < 10 * self.workers:
            logger.warn("under 10 jobs per worker (" + str(self.workers) + ") : consider setting a smaller `batch_words' for smoother alpha decay")
