...
def load_edgelist(file_, undirected=False, leftIsSource=True):
    ...
    G.make_consistent()
    #save_to_edg_list(G, 'edgesnew.txt')
    return G
...
def load_matfile(file_, variable_name="network", undirected=False, weighted=False):
    ...
    ts = from_numpy(mat_matrix, undirected, weighted)
    return ts


def save_to_edg_list(graph, path):
    edges = list()
    for src in graph:
...
def from_adjlist(adjlist):
    ...
    if total_edges == total_lines:
        logger.warning('WARN: May be parsing edge list as adjacency list (# lines == # edges)')
