...
def scoring(embeddings, graph_file, training_percents, network_name="network", labels_name="group", number_shuffles=10,
            columnFilter=None):
    ...
    with open('nodesnew.csv', 'w+') as file:
        file.write('node,category\n')
        for num, entry in enumerate(labels_matrix.toarray()):
            for index, e in numpy.ndenumerate(entry):
                if e == 1:
                    file.write(str(num)+','+str(index[0])+'\n')
...
if __name__ == '__main__':
    parser = ArgumentParser("embedding_scoring", formatter_class=ArgumentDefaultsHelpFormatter,
                            conflict_handler='resolve')
    parser.add_argument("-e", "--embeddings", dest="embeddings", required=True,
                        help="Embeddings file in NumPy .npy format.")
    ...
    parser.add_argument("-t", "--training_percents", type=int, required=True, nargs='+',
